...
__global__ void cudakernel_Compute_r (double* const r, const double* const delta_p, const int arr_size, const ProcComputingCoords procCoords,
    ...
    if (threadId < arr_size){
        int i = threadId % (procCoords.x_cells_num - static_cast<int>(procCoords.right) - static_cast<int>(procCoords.left));
        int j = threadId / (procCoords.x_cells_num - static_cast<int>(procCoords.right) - static_cast<int>(procCoords.left));
